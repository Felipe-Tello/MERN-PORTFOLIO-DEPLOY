...
const Lateral = (props) => {
    ...
    //         value.includes(text) ? true : false
    //     })
    // }
    const checkPriority = (text) => {
    ...
    return (
        <div style={{minWidth:"20%"}}>
            ...
            if (valor) {
                ...
                return (
                <div key={index} style={{display:"flex", flexDirection:"column", alignItems:"center", width:'15vw'}}>
                    ...
                    <div className="nav flex-column nav-pills" aria-orientation="vertical" style={{marginTop:"10%", width:"13vw"}}>
                        <Link to={`/${dashLower(selectedOption)}`}>
                            <button style={{width:"100%"}} className="nav-link">Introduction</button>
                        </Link>
                        {valor.map((value, index) => (
                        <Link key={index} to={`/${dashLower(selectedOption)}/${dashLower(value.normalize("NFD").replace(/[\u0300-\u036f,]/g, ""))}`}>
                            <button style={{width:"100%"}} onClick={() => handleClick(value)} className={activeTab === value ? `nav-link ${assignColor(value)}` : 'nav-link' } >{value}</button>
                        </Link>
                        ))}
                    </div>
                </div>
